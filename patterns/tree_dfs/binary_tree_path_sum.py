...
def binary_tree_path_sum(root: TreeNode | None, sum: int) -> bool:
    if root is None:
        return False

    # if the current root value is a leaf and equal to the sum, we found a path
    if root.value == sum and root.left is None and root.right is None:
        return True

    # recursively call to traverse the left and right sub-tree
    # return true if any of the two recursive call return true
    return binary_tree_path_sum(root.left, sum - root.value) or binary_tree_path_sum(root.right, sum - root.value)

# Time Complexity O(N) where N is total number of nodes
# Space Complexity O(N) in worst case the given tree is a Linked List (every node has one child)
