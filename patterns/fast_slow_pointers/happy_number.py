...
SOLUTION 1
We can use a set to hold all numbers we square and then find if the next number is in the set
Time Complexity O(N)
...
Therefore, we will use the same fast & slow pointer strategy to find the cycle and once the cycle is found, we will see if the cycle is stuck on number ‘1’ to find out if the number is happy or not.
...
def find_happy_number(num):
    ...
    while True:
        slow = find_square(slow) # one step up
        fast = find_square(find_square(fast)) # two steps up

        if slow == fast: # found the cycle
            break

    return slow == 1 # Check if we landed at 1
