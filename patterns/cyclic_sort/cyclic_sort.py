...
To place a number (or an object in general) at its correct index, we first need to find that number. If we first find a number and then place it at its correct place, it will take us 
...
def cyclic_sort(arr):
    ...
    while i < len(arr):
        ...
        if i != value - 1: # Check if value is at correct index
            # Place value at correct place and switch 'objects'
            arr[i], arr[value-1] = arr[value-1], arr[i]
        else: # value is in correct index, move pointer up array
            i += 1
